...
export function getWeddingId() {
  ...
  const found = typeof first === "string" ? first : first?._id;
  return found || (USE_REAL_API ? null : DEMO_WEDDING_ID);
}
export function setSession({ token, user }) {
  if (token) localStorage.setItem("weddingPlusToken", token);
  if (user) localStorage.setItem("weddingPlusUser", JSON.stringify(user));
}
...
function ensureDemoStore() {
  ...
  localStorage.setItem("weddingPlusDemoStore", JSON.stringify({
    ...
    vendors:[
      ...
      {_id:"vendor-2",wedding:DEMO_WEDDING_ID,name:"Studio Lumière",category:"Photo & Vidéo",price:1200,status:"paid"}
    ],
    budget:[
    ...
    timeline:[
      {_id:"timeline-1",wedding:DEMO_WEDDING_ID,title:"Préparatifs",startTime:"2027-06-12T08:00:00.000Z",status:"planned"},
...
function demoUser(name="Glorie",email="demo@weddingplus.fr"){
  ...
  return {success:true,data:{user:{_id:"demo-user",name,email,role:"user",weddings:store.weddings},token:"demo-token",needsOnboarding:false}};
}
function demoDashboard(weddingId){
  const s=readStore(); const wedding=s.weddings.find(w=>w._id===weddingId)||s.weddings[0];
  const filter=name=>(s[name]||[]).filter(x=>x.wedding===wedding._id);
...
async function demoApi(path,options={}){
  ...
  if(path==="/onboarding"&&method==="POST") return {success:true,data:{wedding:readStore().weddings[0],user:getCurrentUser(),suggestions:{tasks:3,budget:2,timeline:2}}};
  if(path.startsWith("/dashboard/")) return demoDashboard(path.split("/").pop());
  if(path.startsWith("/weddings")&&method==="GET") return {success:true,data:readStore().weddings};
  ...
  const weddingQuery=new URLSearchParams(path.split('?')[1]||'').get('wedding');
  if(method==="GET") return {success:true,data:weddingQuery?rows.filter(x=>x.wedding===weddingQuery):rows};
...
export const weddingApi={dashboard:(weddingId)=>api(`/dashboard/${weddingId}`),list:(weddingId=getWeddingId())=>api(`/weddings${weddingId?`?wedding=${weddingId}`:""}`),create:(data)=>api("/weddings",{method:"POST",body:JSON.stringify(data)})};
