...
function ensureDemoStore() {
  ...
  const store = {
    ...
    guests: [
      ...
      {_id:"guest-2",wedding:DEMO_WEDDING_ID,firstName:"Samuel",lastName:"Kouassi",group:"Amis",rsvp:"pending"},
      {_id:"guest-3",wedding:DEMO_WEDDING_ID,firstName:"Marie",lastName:"Lemoine",group:"Amis",rsvp:"confirmed",tableName:"Harmonie"},
      {_id:"guest-4",wedding:DEMO_WEDDING_ID,firstName:"Jean",lastName:"Dupont",group:"Collègues",rsvp:"declined"}
    ],
    vendors: [
  ...
  localStorage.setItem("weddingPlusDemoStore", JSON.stringify(store));
}

function readStore() {
  ensureDemoStore();
  return JSON.parse(localStorage.getItem("weddingPlusDemoStore"));
}
...
function demoUser(name="Glorie", email="demo@weddingplus.fr") {
  ...
  const user = {_id:"demo-user",name,email,role:"user",weddings:store.weddings};
  return {success:true,data:{user,token:"demo-token"}};
}

function demoDashboard(weddingId) {
  const s=readStore();
  const wedding=s.weddings.find(w=>w._id===weddingId)||s.weddings[0];
  ...
  const spent=budget.reduce((a,x)=>a+(x.actual||0),0),estimated=budget.reduce((a,x)=>a+(x.estimated||0),0);
...
async function demoApi(path, options={}) {
  ...
  if (path==="/auth/me") { const u=getCurrentUser()||demoUser().data.user; return {success:true,data:u}; }
  if (path.startsWith("/dashboard/")) return demoDashboard(path.split("/").pop());
  if (path==="/weddings") return {success:true,data:readStore().weddings};
...
export async function api(path, options = {}) {
  ...
  try {
    ...
    if (error instanceof TypeError || String(error.message).toLowerCase().includes("fetch")) {
      console.warn("Wedding+ API indisponible : bascule automatique en mode démo local.");
