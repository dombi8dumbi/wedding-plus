<template>
  <main class="screen dashboard">
    <header class="topbar">
      ...
      <div class="top-actions"><RouterLink to="/alertes" class="icon-btn">♡</RouterLink><div class="avatar">{{ firstName[0] || 'G' }}</div></div>
    </header>
...
import { getCurrentUser,getWeddingId,weddingApi } from '../services/api'
const data=reactive({wedding:null,stats:{},nextTasks:[]})
const user=getCurrentUser(); const firstName=(user?.name||'Glorie').split(' ')[0]
