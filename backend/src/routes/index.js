...
router.get("/dashboard/:weddingId", async (req, res, next) => {
  try {
    ...
    res.json({ success:true, data:{ wedding, stats:{ tasks:{completed:completedTasks,total:tasks.length}, guests:{confirmed:confirmedGuests,total:guests.length}, vendors:{booked:bookedVendors,total:vendors.length}, budget:{spent,estimated,target:wedding.budgetTarget}, unreadAlerts }, nextTasks:tasks.filter((task)=>task.status!=="done").slice(0,5), timeline:timeline.slice(0,8) } });
